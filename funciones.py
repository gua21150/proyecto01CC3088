...
def configure_dataset_directory(csv_files, dataset_dir):
    ...
    sys = int(input("Ingrese el tipo de sistema de su equipo "))
...
def clean_tbl_name(filename):
    ...
    return '{0}'.format(clean_tbl_name.split('.')[0])  # nombre de la tabla

"""Cambiar los nombres de las columnas a un formato mas limpio"""
def clean_colname(data_frame):
    data_frame.columns = [x.lower().replace(" ", "_").replace("?", "")\
                        .replace("-", "_").replace(r"/", "_").replace("\\", "_").replace("%", "")\
